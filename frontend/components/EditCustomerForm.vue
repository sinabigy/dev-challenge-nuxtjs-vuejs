<template>
  <v-card>
      ...
      <v-form ref="form">
        ...
        >
          ...
          <v-date-picker v-model="customer.purchase_date" no-title scrollable>
            ...
            <v-btn text color="primary" @click="$refs.menu.save(customer.purchase_date)">
              OK
            </v-btn>
          </v-date-picker>
        </v-menu>
    ...
    <v-card-actions>
      <v-btn :disabled="lock" @click="updateCustomer()" color="primary">
        Save
      </v-btn>
    </v-card-actions>
  </v-card>
...
export default {
  ...
  // Watches the customer_id prop and fetches the corresponding customer when it changes.
  watch: {
    customer_id(newValue) {
      this.getCustomer(newValue);
    },
    // Watches the customer.purchase_date property and updates the formattedDateTime property when it changes.
  ...
  methods: {
    ...
    // Fetches a single customer from the server using an axios GET request.
    async getCustomer() {
      // Checks that the customer_id is valid, to avoid sending a request on closing the dialog.
      if (!this.customer_id) {
        return;
      }
      const employees = await this.$axios.$get('/employee/employees');
      const data = await this.$axios.$get(`/customer/customer/${this.customer_id}`);

      let employeeLookup = employees.reduce((acc, employee) => {
        acc[employee.id] = employee;
        return acc;
      }, {});

      // Sets the customer object to the fetched data.
      this.customer = {
        ...
        employee_name : employeeLookup[data.employee_id] || 'No Employee'
      };
     
    },
    // Updates an existing customer on the server using an axios PUT request.
    async updateCustomer() {
      // Validates the form before submitting the request.
      if (this.$refs.form.validate()) {
        this.lock = true;
        await this.$axios.$put(`/customer/customer/${this.customer_id}`, this.customer);
        this.lock = false;
        ...
        // Emits a 'save' event to notify the parent component that the customer was updated.
        this.$emit("save");
      }
    },
  },
  // Initializes the component by fetching the list of employees from the server and mapping them to an array of objects that can be used to populate the employee select field, and by fetching the customer with the corresponding customer_id prop.
  async created() {
